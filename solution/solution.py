...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for rank, relevant in enumerate(relevant_at_rank, start=1):
            if relevant:
                hits += 1
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "You are an impartial evaluator. Score each criterion from 0.0 to 1.0. "
        ...
        try:
            ...
            scores = {
                ...
                if criterion in parsed else 0.5
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if isinstance(first, (int, float)) and isinstance(second, (int, float)):
                first_wins.append(first > second)
        return {
            "positional_bias": bool(first_wins) and all(first_wins),
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        return {
            ...
            "avg_context_recall": average(recalls),
            "avg_context_precision": average(precisions),
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures, start=1):
            ...
            lines.append(
                f"| {failure_id} | {failure.failure_type or 'unknown'} | "
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination", 0):
            suggestions.append("Add a grounding check that removes claims unsupported by retrieved context.")
        if categories.get("irrelevant", 0) or categories.get("off_topic", 0):
            suggestions.append("Clarify the answer prompt with intent-focused instructions and relevant few-shot examples.")
        if categories.get("incomplete", 0):
            suggestions.append("Retrieve broader evidence and require the generator to cover every condition and exception.")
        fallback = [
            "Review low-scoring traces and add representative failures to the regression dataset.",
            "Measure retrieval recall and precision after each retriever, chunking, or prompt change.",
            "Use a calibrated LLM judge alongside deterministic metrics for release decisions.",
        ]
